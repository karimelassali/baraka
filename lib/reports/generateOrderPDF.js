...
export const generateOrderPDF = (order) => {
    ...
    logoImg.onload = () => {
        ...
        doc.text('Sistema di Gestione Ordini', 45, 32);

        // Order Details (Right side)
        doc.setFontSize(10);
        doc.text(`Ordine #: ${order.order_number}`, pageWidth - 15, 20, { align: 'right' });
        doc.text(`Data: ${new Date(order.created_at).toLocaleDateString('it-IT')}`, pageWidth - 15, 26, { align: 'right' });
        ...
        autoTable(doc, {
            startY: tableStartY,
            head: [['Nome Prodotto', 'Quantità', 'Note']],
            body: tableBody,
            theme: 'grid',
        ...
        doc.text('Generato da Baraka Admin System', pageWidth / 2, 280, { align: 'center' });

        // Save
        doc.save(`Ordine_${order.order_number}.pdf`);
    };
