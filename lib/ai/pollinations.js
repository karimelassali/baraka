...
- **Formatting**: Use Markdown for all outputs. Use **bold** for emphasis, lists for data, and tables for structured info.
`;
...
export async function sendMessageToAI(messages) {
    try {
        ...
        // 2. Prepare messages array with sliding window memory
        // We always want the system prompt first.
        // Then we take the last 10 messages from the history to keep context small but relevant.
        // The 'messages' argument is expected to be the full history or already sliced.
        // However, to be safe, we'll slice it here if it's too long, excluding the system prompt if it was passed (which it shouldn't be usually, but let's handle it).

        // Filter out any previous system messages to avoid duplication if they were stored in state
