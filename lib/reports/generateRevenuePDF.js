...
export const generateRevenuePDF = async (data, month, year) => {
    ...
    try {
        ...
        doc.text(`Monthly Revenue Report – ${monthName} ${year}`, 40, 30);
        ...
        doc.setFontSize(14);
        doc.setTextColor(...textColor);
        doc.text(`Monthly Revenue Report – ${monthName} ${year}`, 14, 30);
    }

    // Table Data
    const tableColumn = ["Date", "Total Revenue", "Cash", "Card", "Ticket"];
    const tableRows = [];
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Generated on ${new Date().toLocaleDateString()}`, 200, 290, { align: "right" });
    }
