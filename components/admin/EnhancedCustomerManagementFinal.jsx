  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        onSave && onSave(result); // Pass the new customer to parent
        setTimeout(() => {
          onClose();
          setStatus({ type: '', message: '' });
...
export default function EnhancedCustomerManagement() {
  ...
  const loadCustomers = async (reset = false) => {
    ...
    try {
      let url = `/api/admin/customers?limit=50&offset=${reset ? 0 : offset}&sort_by=created_at&sort_order=desc`;
      ...
      if (response.ok) {
        ...
        setHasMore((data.customers || data).length === 50);
        if (!reset) {
